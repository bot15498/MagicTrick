...
public class MeshGenerator : MonoBehaviour
{
    ...
    if (lightLayer != -1) // Check if the "Light" layer exists
    {
        colorCamera.cullingMask = (1 << (int)Mathf.Log(terrainLayer.value, 2)) | (1 << lightLayer);
    }
    else
    {
        Debug.LogWarning("Layer 'Light' does not exist. Make sure to create it in the Layer settings.");
        colorCamera.cullingMask = 1 << (int)Mathf.Log(terrainLayer.value, 2); // Default to "Terrain" layer
    }
