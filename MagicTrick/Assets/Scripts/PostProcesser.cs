...
public class PostProcesser : MonoBehaviour
{
    ...
    public GameObject _grassCamera;
    ...
    void OnEnable()
{
    ...
    if (grassState == ShaderState.On && _grassCameraObject == null)
    {
        ...
        // Exclude the "Light" layer
        int lightLayer = LayerMask.NameToLayer("Light");
        if (lightLayer != -1) // Check if "Light" layer exists
        {
            grassCamera.cullingMask &= ~(1 << lightLayer); // Remove "Light" layer from cullingMask
        }
        else
        {
            Debug.LogWarning("Layer 'Light' does not exist. Ensure it is created in Layer settings.");
        }

        // Add Post-Processing Layer
        PostProcessLayer postProcessLayer = _grassCameraObject.AddComponent<PostProcessLayer>();
        postProcessLayer.volumeLayer = LayerMask.GetMask("Default");
        postProcessLayer.antialiasingMode = PostProcessLayer.Antialiasing.None;

        // Add Post-Processing Volume
        PostProcessVolume postProcessVolume = _grassCameraObject.AddComponent<PostProcessVolume>();
        postProcessVolume.profile = postProcessProfile;
        postProcessVolume.isGlobal = true;

        // Debugging: Verify the final culling mask
        Debug.Log($"GrassCamera Final CullingMask: {grassCamera.cullingMask}");
    }
}
